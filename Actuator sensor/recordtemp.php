<?php 
	...
	if(isset($_GET['t'])) { // Check if temperature is present or not
		
	     $str = '';     // intialize empty string

	     if(file_exists('tempDataenc.xml')) { // if the xml file already exists then 
						// read it
		$str = file_get_contents('tempDataenc.xml'); // get all the current data
	     }
		
	     if(strlen($str) == 0) {	// initialize the variable to the empty xml if 
